...
class FinancePage(QWidget):
    def __init__(self, database, parent=None):
        ...
        self.init_ui()
        self.update_installments_status()

    def setup_table_header(self, table, stretch_mode=None, stretch_columns=None):
        """تنسيق رأس الجدول
        Args:
            table: جدول QTableWidget المراد تنسيقه
            stretch_mode: نمط تمدد الأعمدة (ResizeToContents أو Stretch)
            stretch_columns: قائمة بأرقام الأعمدة التي سيتم تمديدها
        """
    ...
    def save_installment(self):
        """حفظ قسط جديد"""
        try:
            if not self.total_amount.text().strip():
                UIHelper.show_warning(self, "تنبيه", "يرجى إدخال إجمالي المبلغ")
                return
            ...
            success, installment_id, error = self.installments_manager.save_installment(
                ...
                float(self.total_amount.text()),
                float(self.down_payment.text() or "0"),
            ...
            UIHelper.show_warning(self, "تنبيه", "يرجى إدخال مبالغ صحيحة")
    ...
    def create_invoice(self):
        ...
        try:
            ...
            if success:
                UIHelper.show_success(self, "نجاح", f"تم إنشاء الفاتورة رقم {invoice_number} بنجاح")
    ...
    def load_invoices(self):
        ...
        for i, inv in enumerate(invoices):
            number, car, client, date, amount, method, status = inv
            
            self.invoices_table.setItem(i, 0, QTableWidgetItem(number))
    ...
    def generate_report(self):
        ...
        if report_type == "تقرير الإيرادات والمصروفات":
            success, results, summary = self.reports_manager.generate_financial_report(start_date, end_date)
            if success:
                self.show_financial_report(results, summary)
        elif report_type == "تقرير الأقساط":
            success, results, summary = self.reports_manager.generate_installments_report(start_date, end_date)
            if success:
                self.show_installments_report(results, summary)
        elif report_type == "تقرير المبيعات":
            success, results, summary = self.reports_manager.generate_sales_report(start_date, end_date)
            if success:
                self.show_sales_report(results, summary)
        else:  # تقرير العملاء
            success, results, summary = self.reports_manager.generate_clients_report(start_date, end_date)
            if success:
    ...
    def update_installments_status(self):
        """تحديث حالة الأقساط"""
        try:
            ...
            UIHelper.show_error(self, "خطأ", f"حدث خطأ في تحديث حالة الأقساط: {str(e)}")
